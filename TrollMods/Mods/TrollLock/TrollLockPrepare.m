...
void trollLockPrepare(void) {
    for (int i = 1; i <= 40; i++) {
        NSString *trollFilePath = [NSString stringWithFormat:@"trollformation%d", i];
        NSString *trollFile = [[NSBundle mainBundle] pathForResource:trollFilePath ofType:@"png"];
        NSString *trollReplace = [NSString stringWithFormat:@"trolling%dx", i];

        fileContents = [fileContents stringByReplacingOccurrencesOfString:trollReplace withString:trollFile];
    }
